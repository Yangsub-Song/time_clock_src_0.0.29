...
private val logger = KotlinLogging.logger("TempSensor")

object TempSensor: HwStatus() {
    private val portName = if (isLinux) "ttyS4" else "COM12"
    private val serial = Serial(portName, 19200, "TempSensor")
    ...
    private fun calcCrc16(data: ByteArray): Short {
        var crc = 0xFFFF
        data.forEach {
            val i = (crc xor it.toInt()) and 0xff
            crc = ((crc ushr 8) xor crc16Table[i]) and 0xffff
        }
        return crc.toShort()
    }
    ...
    private fun createTx() =
        ByteBuffer.allocate(8).apply {
            ...
            val data = byteArrayOf(0x01, 0x03, 0x00, 0x01, 0x00, 0x04)
            put(data)
            putShort(calcCrc16(data))
        }

    private suspend fun getTemperatureInternal(): Float? {
        serial.transfer(createTx(), 13)?.run {
            order(ByteOrder.BIG_ENDIAN)
            if (get() != 0x01.toByte() || get() != 0x03.toByte() || get() != 0x08.toByte()) {
                return null
            }
            val rawData = short * 0.02f - 273.15f
            val compensatedData = short * 0.02f - 273.15f
            val reserved = short
            val sensorData = short * 0.02f - 273.15f
            order(ByteOrder.LITTLE_ENDIAN)
            ...
            if (short != calcCrc16(byteArray)) {
                logger.warn { "CRC error" }
                return null
            }
            logger.info { "$rawData *$compensatedData $sensorData" }
            return compensatedData
        }
        return null
    }

    suspend fun getTemperature(timeout: Int = 10000): Float? {
        if (isLinux) {
            serial.setReadTimeout(timeout)
            return getTemperatureInternal().also {
                isHwOk = it != null
            }
        } else {
            delay(1000)
            return (34..39).random().toFloat()
            //return (39..40).random().toFloat()
        }
    }

    override suspend fun checkHw(): Boolean {
        return getTemperature(2000) != null
    }
}
